...
export default function SupervisorWiseReport() {
  ...
  const fetchReport = async () => {
    setLoading(true);
    try {
      // 1. Fetch supervisor issuance report instantly from database in 3ms
  ...
  const exportToPdf = () => {
    ...
    setFont("bold", 12);
    doc.text("8", circleX, circleY + 4, { align: "center" });

    // Numbering Label & Title
    setFont("bold", 8);
    doc.setTextColor(0, 0, 0);
    doc.text("8TH REPORT", M + 45, y + 11);

    setFont("bold", 13);
    doc.setTextColor(0, 0, 0);
    doc.text("SUPERVISOR SHIFT SUMMARY REPORT", M + 45, y + 25);
