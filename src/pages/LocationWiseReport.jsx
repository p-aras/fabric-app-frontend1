...
import {
  ...
  Layers, Scale, Tag, Users, Warehouse
} from 'lucide-react';
...
export default function LocationWiseReport() {
  ...
  const fetchReport = async () => {
    setLoading(true);
    try {
      const response = await store.getLocationIssuanceReport(startDate, endDate);
      ...
      console.error("Error loading location wise report:", err);
  ...
  // Get unique locations and supervisors for filters
  const uniqueLocations = useMemo(() => {
    const locs = reportData.map(item => item.location).filter(Boolean);
  ...
  }, [reportData, searchTerm, selectedLocation, selectedSupervisor]);
  ...
  const stats = useMemo(() => {
    return filteredData.reduce((acc, curr) => {
      ...
      acc.locations.add(curr.location);
  ...
  // Prepare rolls by location chart data
  const locationChartData = useMemo(() => {
    const locMap = {};
    filteredData.forEach(item => {
      const loc = item.location || 'Not Assigned';
      locMap[loc] = (locMap[loc] || 0) + (item.rolls || 0);
    });
    
    return Object.entries(locMap)
      .map(([location, rolls]) => ({ location, rolls }))
      .sort((a, b) => b.rolls - a.rolls)
      .slice(0, 10); // Display top 10 locations
  }, [filteredData]);
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Location Issuance Report");
    XLSX.writeFile(wb, `Location_Issuance_Report_${startDate}_to_${endDate}.xlsx`);
  };
  ...
  const exportToPdf = () => {
    ...
    doc.text("ACTIVE LOCATIONS", M + 270, y + 16);
    doc.text("ACTIVE SUPERVISORS", M + 405, y + 16);

    setFont("bold", 11);
    doc.text(`${stats.totalRolls} Rolls`, M + 10, y + 34);
    doc.text(`${stats.totalWeight.toFixed(2)} KG`, M + 140, y + 34);
    doc.text(`${stats.locations.size}`, M + 270, y + 34);
    doc.text(`${stats.supervisors.size}`, M + 405, y + 34);
    ...
    const headers = [
      ...
      { label: "Storage Location", w: 100, align: "left" },
      { label: "Lot Number", w: 65, align: "left" },
      { label: "Fabric Description", w: 110, align: "left" },
      { label: "Shade", w: 60, align: "left" },
      { label: "Supervisor", w: 75, align: "left" },
    ...
    filteredData.forEach((item, idx) => {
      ...
      const rowVals = [
        ...
        item.location || "—",
        item.lotNumber || "—",
        String(item.fabric || "—").length > 22 ? String(item.fabric).substring(0, 19) + "..." : (item.fabric || "—"),
        item.shade || "—",
        item.supervisor || "—",
    ...
    doc.save(`Location_Wise_Issuance_Report_${startDate}_to_${endDate}.pdf`);
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, maxWidth: '2000px', margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <div>
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: 'var(--text-primary)' }}>Location Wise Issuance Report</h2>
          <p style={{ margin: '4px 0 0 0', fontSize: 13, color: 'var(--text-secondary)' }}>
            Report showing storage locations (rooms, halls, racks, shelves) from which fabric rolls were issued.
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16 }}>
        ...
        <div className="card" style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16, border: '1px solid var(--border)' }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)' }}>Active Locations</div>
            <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--text-primary)', marginTop: 2 }}>
              {stats.locations.size} <span style={{ fontSize: 12, fontWeight: 600 }}>Locations</span>
            </div>
          </div>
        </div>
      ...
      {filteredData.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(450px, 1fr))', gap: 16 }}>
          ...
          {/* ROLLS BY LOCATION */}
          <div className="card" style={{ border: '1px solid var(--border)', padding: '20px 24px' }}>
            <h3 style={{ margin: '0 0 16px 0', fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}>Top Storage Locations by Rolls Issued</h3>
            <div style={{ width: '100%', height: 260 }}>
              <ResponsiveContainer>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 12, color: 'var(--text-secondary)', fontWeight: 600 }}>Location:</span>
              <select
              ...
              {loading ? (
                ...
                filteredData.map((item, idx) => (
                  <tr key={item.id} className="hover-row" style={{ borderBottom: '1px solid var(--border-light)' }}>
                    ...
                    <td>
                      <span style={{ display: 'inline-block', padding: '3px 6px', borderRadius: 4, backgroundColor: 'var(--primary-light)', fontSize: 11, fontWeight: 600 }}>
                        {item.supervisor}
                      </span>
                    </td>
